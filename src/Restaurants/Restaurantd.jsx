...
const { count, setCount, cart, setCart } = useCount();
    ...
    return (
        restaurant.map((card) => {
            return (
                <div key={card.id} className='border-[1px] border-gray-300 w-[350px] h-[450px] pl-[25px] pt-[30px] rounded-lg bg-slate-900 text-slate-50'>
                    ...
                    <div className='bg-slate-800 w-[200px] mx-[4px] flex justify-between mt-[15px]'>
                        <div>{card.type}</div>
                    </div>
                    <div className='flex justify-between mx-[10px] mt-[25px]'>
                        <span className='text-blue-500'>{card.price}/meal</span>
